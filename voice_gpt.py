...
openai.api_key = "sk-Rj1NRalp9LrVvTPh"
# Initialize the text to speech engine
engine = pyttsx3.init()
voices = engine.getProperty('voices')
compatible_voices = [
    voice for voice in voices if "HKEY_LOCAL_MACHINE" not in voice.id]

for voice in voices:
    print(voice.name)
    
# Set the voice to the first compatible voice
if compatible_voices:
    engine.setProperty('voice', compatible_voices[0].id)
else:
